...
def get_streak_leaderboard():
    """Returns users with an active streak, sorted by current_streak DESC.

    Only includes users whose last_project_post_date is within the active window
    (posted today, yesterday, or the day before - i.e. still within grace period).
    This prevents stale streaks from appearing on the leaderboard when a user
    stops posting but has not yet triggered a reset by posting again.
    """
    from datetime import date, timedelta
    today = get_vancouver_today()
    today_date = date.fromisoformat(today)
    cutoff = (today_date - timedelta(days=2)).isoformat()

    conn = get_db_connection()
    users = conn.execute(
        '''SELECT * FROM users
           WHERE current_streak > 0
           AND last_project_post_date >= ?
